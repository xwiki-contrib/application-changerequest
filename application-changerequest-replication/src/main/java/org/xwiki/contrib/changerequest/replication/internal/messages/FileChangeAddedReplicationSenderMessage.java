...
@Component(roles = FileChangeAddedReplicationSenderMessage.class)
public class FileChangeAddedReplicationSenderMessage extends AbstractChangeRequestEventReplicationSenderMessage
{
    ...
    public void initialize(ChangeRequestFileChangeAddedRecordableEvent event, DocumentReference modifiedDocument)
    {
        this.saveContextUser();
        this.putCustomMetadata(CHANGE_REQUEST_ID_PARAMETER,
            event.getChangeRequestId());
        this.putCustomMetadata(FILE_CHANGE_ID, event.getFileChangeId());
        this.putCustomMetadata(MODIFIED_DOCUMENT, this.entityReferenceSerializer.serialize(modifiedDocument));
    }
}
