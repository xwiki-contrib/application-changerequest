...
/**
 * Tests for {@link StaleChangeRequestEventReceiver}.
 *
 * @version $Id$
 */
@ComponentTest
class StaleChangeRequestEventReceiverTest
{
    @InjectMockComponents
    private StaleChangeRequestEventReceiver eventReceiver;
    ...
    void receive() throws Exception
    {
        ...
        doAnswer(invocation -> {
            StaleChangeRequestRecordableEvent event = invocation.getArgument(0);
            assertEquals(crId, event.getChangeRequestId());
            return null;
        }).when(this.recordableEventNotifier)
            .notifyChangeRequestRecordableEvent(any(StaleChangeRequestRecordableEvent.class), eq(dataDoc));
