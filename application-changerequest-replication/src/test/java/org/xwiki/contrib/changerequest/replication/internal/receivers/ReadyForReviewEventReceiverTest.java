...
/**
 * Tests for {@link ReadyForReviewEventReceiver}.
 *
 * @version $Id$
 */
@ComponentTest
class ReadyForReviewEventReceiverTest
{
    @InjectMockComponents
    private ReadyForReviewEventReceiver eventReceiver;
    ...
    void receive() throws Exception
    {
        ...
        String targets = "user1,user2,user3";
        when(this.messageReader.getMetadata(message, "TARGETS", true)).thenReturn(targets);
        ...
        doAnswer(invocation -> {
            ChangeRequestReadyForReviewTargetableEvent event = invocation.getArgument(0);
            assertEquals(new HashSet<>(List.of("user1", "user2", "user3")), event.getTarget());
            return null;
        }).when(this.recordableEventNotifier)
            .notifyChangeRequestRecordableEvent(any(ChangeRequestReadyForReviewTargetableEvent.class), eq(dataDoc));
