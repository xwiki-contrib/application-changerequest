...
/**
 * Tests for {@link DocumentModifiedInChangeRequestEventReceiver}.
 *
 * @version $Id$
 */
@ComponentTest
class DocumentModifiedInChangeRequestEventReceiverTest
{
    @InjectMockComponents
    private DocumentModifiedInChangeRequestEventReceiver eventReceiver;
    ...
    void receive() throws Exception
    {
        ...
        doAnswer(invocation -> {
            DocumentModifiedInChangeRequestEvent event = invocation.getArgument(0);
            assertEquals(crId, event.getChangeRequestId());
            return null;
        }).when(this.recordableEventNotifier)
            .notifyChangeRequestRecordableEvent(any(DocumentModifiedInChangeRequestEvent.class), eq(dataDoc));
