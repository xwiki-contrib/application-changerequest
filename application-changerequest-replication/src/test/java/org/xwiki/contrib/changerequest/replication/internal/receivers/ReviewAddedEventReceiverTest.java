...
/**
 * Tests for {@link ReviewAddedEventReceiver}.
 *
 * @version $Id$
 */
@ComponentTest
class ReviewAddedEventReceiverTest
{
    @InjectMockComponents
    private ReviewAddedEventReceiver eventReceiver;
    ...
    void receive() throws Exception
    {
        ...
        String reviewId = "review4343";
        when(this.messageReader.getMetadata(message, "REVIEW_ID", true)).thenReturn(reviewId);
        ...
        doAnswer(invocation -> {
            ChangeRequestReviewAddedRecordableEvent event = invocation.getArgument(0);
            assertEquals(crId, event.getChangeRequestId());
            assertEquals(reviewId, event.getReviewId());
            return null;
        }).when(this.recordableEventNotifier)
            .notifyChangeRequestRecordableEvent(any(ChangeRequestReviewAddedRecordableEvent.class), eq(dataDoc));
