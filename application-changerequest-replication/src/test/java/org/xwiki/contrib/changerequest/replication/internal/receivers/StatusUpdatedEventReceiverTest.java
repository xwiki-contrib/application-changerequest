...
/**
 * Tests for {@link StatusUpdatedEventReceiver}.
 *
 * @version $Id$
 */
@ComponentTest
class StatusUpdatedEventReceiverTest
{
    @InjectMockComponents
    private StatusUpdatedEventReceiver eventReceiver;
    ...
    void receive() throws Exception
    {
        ...
        when(this.messageReader.getMetadata(message, "OLD_STATUS", true)).thenReturn("READY_FOR_REVIEW");
        when(this.messageReader.getMetadata(message, "NEW_STATUS", true)).thenReturn("MERGED");
        ...
        doAnswer(invocation -> {
            ChangeRequestStatusChangedRecordableEvent event = invocation.getArgument(0);
            assertEquals(crId, event.getChangeRequestId());
            assertEquals(ChangeRequestStatus.READY_FOR_REVIEW, event.getOldStatus());
            assertEquals(ChangeRequestStatus.MERGED, event.getNewStatus());
            return null;
        }).when(this.recordableEventNotifier)
            .notifyChangeRequestRecordableEvent(any(ChangeRequestStatusChangedRecordableEvent.class), eq(dataDoc));
