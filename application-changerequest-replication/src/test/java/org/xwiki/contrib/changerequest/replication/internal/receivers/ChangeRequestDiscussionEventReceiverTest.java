...
/**
 * Tests for {@link ChangeRequestDiscussionEventReceiver}.
 *
 * @version $Id$
 */
@ComponentTest
class ChangeRequestDiscussionEventReceiverTest
{
    @InjectMockComponents
    private ChangeRequestDiscussionEventReceiver eventReceiver;
    ...
    void receive() throws Exception
    {
        ...
        String discussionType = "discussionType24";
        String discussionRef = "ref24";
        String messageRef = "ref5433";

        when(this.messageReader.getMetadata(message, "DISCUSSION_TYPE", true)).thenReturn(discussionType);
        when(this.messageReader.getMetadata(message, "DISCUSSION_REFERENCE", true)).thenReturn(discussionRef);
        when(this.messageReader.getMetadata(message, "MESSAGE_REFERENCE", true)).thenReturn(messageRef);
        ...
        doAnswer(invocation -> {
            ChangeRequestDiscussionRecordableEvent event = invocation.getArgument(0);
            assertEquals(crId, event.getChangeRequestId());
            assertEquals(discussionType, event.getDiscussionType());
            assertEquals(discussionRef, event.getDiscussionReference());
            assertEquals(messageRef, event.getMessageReference());
            return null;
        }).when(this.recordableEventNotifier)
            .notifyChangeRequestRecordableEvent(any(ChangeRequestDiscussionRecordableEvent.class), eq(dataDoc));
