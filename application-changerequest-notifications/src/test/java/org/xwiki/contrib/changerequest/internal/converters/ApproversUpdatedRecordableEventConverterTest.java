...
/**
 * Tests for {@link ApproversUpdatedRecordableEventConverter}.
 *
 * @version $Id$
 * @since 0.11
 */
@ComponentTest
class ApproversUpdatedRecordableEventConverterTest
{
    @InjectMockComponents
    private ApproversUpdatedRecordableEventConverter converter;
    ...
    {
        ApproversUpdatedTargetableEvent recordableEvent = mock(ApproversUpdatedTargetableEvent.class);
        Event event = mock(Event.class);
        String source = "a source";
        int data = 42;
        when(this.defaultConverter.convert(recordableEvent, source, data)).thenReturn(event);

        String eventName = ApproversUpdatedTargetableEvent.EVENT_NAME;
