...
/**
 * Default converter for {@link ChangeRequestReadyForReviewTargetableEvent}.
 *
 * @version $Id$
 * @since 0.11
 */
...
public class ChangeRequestReadyForReviewRecordableEventConverter implements RecordableEventConverter
{
    @Inject
    ...
    {
        Event result = this.recordableEventConverter.convert(recordableEvent, source, data);
        result.setType(ChangeRequestReadyForReviewTargetableEvent.EVENT_NAME);
        return result;
    }
    ...
    {
        return Collections.singletonList(new ChangeRequestReadyForReviewTargetableEvent());
    }
}
