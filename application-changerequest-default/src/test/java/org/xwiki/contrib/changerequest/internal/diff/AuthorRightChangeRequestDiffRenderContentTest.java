...
/**
 * Tests for {@link AuthorRightChangeRequestDiffRenderContent}.
 *
 * @version $Id$
 * @since 1.4.4
 * @since 1.5
 */
@ComponentTest
class AuthorRightChangeRequestDiffRenderContentTest
{
    @InjectMockComponents
    private AuthorRightChangeRequestDiffRenderContent diffRenderContent;
    ...
    void getRenderedContent() throws XWikiException, ChangeRequestException
    {
        ...
        DocumentReference fileChangeAuthorDocRef = mock(DocumentReference.class);
        UserReference fileChangeAuthor = mock(UserReference.class);
        when(fileChange.getAuthor()).thenReturn(fileChangeAuthor);
        when(this.userReferenceConverter.convert(fileChangeAuthor)).thenReturn(fileChangeAuthorDocRef);
        ...
        assertEquals(expectedResult, this.diffRenderContent.getRenderedContent(document, fileChange));
        verify(documentAuthors).setContentAuthor(fileChangeAuthor);
        verify(documentAuthors).setEffectiveMetadataAuthor(fileChangeAuthor);
        verify(this.context).setDoc(cloneDoc);
        verify(this.context).setDoc(currentDoc);
        verify(this.context).setUserReference(fileChangeAuthorDocRef);
